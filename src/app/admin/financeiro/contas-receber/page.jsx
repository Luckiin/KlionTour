...
import { 
  ArrowUpCircle, Plus, Search, Filter, ChevronLeft, ChevronRight, 
...
export default function ContasReceber() {
  ...
  const { lancamentos, isLoading, mutate } = useLancamentos({ tipo: 'receita', mes: mesISO, limit: 1000 });
  ...
  async function toggleStatus(id, statusAtual) {
    ...
    try {
      ...
      toast.success(novoStatus === "pago" ? "Recebido com sucesso!" : "Reaberto");
      mutate();
  ...
  const filtrados = useMemo(() => {
    return lancamentos.filter(l => 
      ...
      l.users?.name?.toLowerCase().includes(busca.toLowerCase())
    );
  }, [lancamentos, busca]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-brand-900 dark:text-white flex items-center gap-2">
            <ArrowUpCircle size={20} className="text-emerald-500" /> Contas a Receber
          </h1>
          <p className="text-sm text-steel-500 mt-1">Gestão de faturamento KlionTour</p>
        </div>
        <button onClick={() => setModal({ tipo: 'receita' })} className="btn-primary px-6 py-3 text-xs font-black uppercase tracking-widest flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { l: "Recebido no Mês", v: fmtBRL(totalRecebido), c: "text-emerald-500", bg: "bg-emerald-500/5" },
          { l: "A Receber (Aberto)", v: fmtBRL(totalPendente), c: "text-brand-500", bg: "bg-brand-500/5" },
          { l: "Total Previsto", v: fmtBRL(totalRecebido + totalPendente), c: "text-steel-400", bg: "bg-surface-subtle" },
        ].map(k => (
              ...
              {MESES_ABREV.map((m, i) => (
                <button key={m} onClick={() => setMes(i)}
                  className={cn("px-4 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all",
                    i === mes ? "bg-brand-500 text-white shadow-lg shadow-brand-500/20" : "text-steel-400 hover:text-brand-900 hover:bg-brand-500/5"
                  )}
                >
                  {m}
                </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-subtle/30 dark:bg-surface-dark-subtle/10 border-b border-surface-border dark:border-surface-dark-border">
                {["Status", "Descrição", "Vencimento", "Valor", "Cliente", ""].map(h => (
            ...
            <tbody className="divide-y divide-surface-border dark:divide-surface-dark-border">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="p-20 text-center text-xs font-bold text-steel-400 uppercase tracking-widest">Nenhuma receita no período</td></tr>
              ) : (
                filtrados.map(l => {
                  ...
                  return (
                    <tr key={l.id} className={cn("table-row-hover", pago && "opacity-60")}>
                      <td className="p-6">
                        <button onClick={() => toggleStatus(l.id, l.status)}
                          className={cn("w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all",
                            pago ? "bg-emerald-500 border-emerald-500 text-white" : "border-surface-border dark:border-surface-dark-border hover:border-emerald-500"
                          )}>
                      ...
                      <td className="p-6 font-black text-sm whitespace-nowrap" style={{ color: pago ? "inherit" : "#10b981" }}>
                        {fmtBRL(pago ? (l.valor_pago || l.valor) : l.valor)}
                      </td>
                      <td className="p-6 text-xs text-steel-500 font-medium">
                        {l.users?.name || l.fornecedor || "—"}
                      </td>
                      <td className="p-6 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => setModal({ tipo: 'receita', original: l })} className="p-2 rounded-xl text-steel-400 hover:text-brand-500 hover:bg-brand-500/10 transition-all"><Pencil size={16} /></button>
